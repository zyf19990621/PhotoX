...
//            throw PhotoCollectionError.removeAllError(error)
    ...
    private func refreshTrashPhotoAssets() async {
        let trashAssetCollection = PHAssetCollection.transientAssetCollection(with: trashPHAssets, title: "废纸篓")
        let fetchOptions = PHFetchOptions()
        fetchOptions.sortDescriptors = [NSSortDescriptor(key: "creationDate", ascending: false)]
        if let fetchResult = (PHAsset.fetchAssets(in: trashAssetCollection, options: fetchOptions) as AnyObject?) as? PHFetchResult<PHAsset> {
            await MainActor.run {
                trashPhotoAssets = PhotoAssetCollection(fetchResult)
                logger.debug("PhotoCollection trashPhotoAssets refreshed: \(self.trashPhotoAssets.count)")
            }
        }
    }
}
...
fileprivate let logger = Logger(subsystem: "com.zyf19990621.git.PhotoX", category: "PhotoCollection")
